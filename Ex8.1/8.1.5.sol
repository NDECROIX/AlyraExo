...
contract CanalDePaiment {
    ...
    function fermeture( uint _nonce, 
                        uint _equilibreA,
                        uint _equilibreB,
                        bytes memory _signature ) public onlyParties() {
    ...
    function signatureValide(bytes32 _message, bytes memory _signature) public pure returns (address) {
    ...
    function retraitFonds() public  etatCanal(EtatCanal.ENCOURSFERMETURE) 
                                    onlyParties() {
        require(block.number > blocFermeture);
        etat = EtatCanal.FERME;
        partieA.transfer(equilibreA);
        selfdestruct(partieB);
